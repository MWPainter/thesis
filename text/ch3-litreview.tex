...
\chapter{\label{ch:3-litreview}Related Work}

    \minitoc



    \todo{Uncomment the lots of messy notes in this chapter and clean up}




MAKE SURE FOLLOWING SECTION LABELS 
...
\label{sec:3-1-mab}
    ...
    \paragraph{Sequential Halving}

        \todo{Cite and summarise}
...
\label{sec:3-2-rl}
    ...
    \paragraph{Entropy Regularisation}
        ...
        \todo{cite some examples of entropy regularisation. Say that only a one step entropy is used}


    \paragraph{Maximum Entropy Reinforcement Learning}

        \todo{cite and describe the deep maximum entropy rl papers}

        \todo{give the background properly too?}
...
%     \todo{should add some boltzmann search stuff here too}

%     \todo{Change chapter title to related work rather than literature review}
...
% THTS + RTDP to the boltzmann exploration stuff too
% \todo{Update ch4 section introducing BTS to account for it not being original}
...
%         Entropy widely used in RL literature. Most commonly used for regularization when learning a policy \cite{ppo,a3c,reinforce_ment}. This is significantly different to the maximum entropy setting, where entropy is used in the loss function to prevent the policy from degenerating into a deterministic policy. While it is motivated similarly, promoting the policy to remain stochastic and enable exploration. 

%         The significant difference between \textit{entropy regularization} and \textit{maximum entropy reinforcement learning} is that the former is that from some state $s_t$, regularization only includes the entropy of the policy at $s_t$, while the latter includes the entropy of the policy at all states $s_t,s_{t+1},...$ in the objective/loss function.

%         Probably write this a bit better, say that entropy regularization dates back to the REINFORCE/MENT paper, but is consistently used to the modern day.


%         Old entropy stuff from ch4 before rewrite:
%         Loosely, the maximum entropy principle says that given partial knowledge about a probability distribution, the least biased estimate of the probability distribution is the one that maximises the entropy \cite{max_ent}. This principle can naturally be applied to inverse reinforcement learning \cite{max_ent_irl,max_ent_irl_thesis}, where the goal is to learn the transition/reward functions from a set of expert trajectories.

%     In contrast, in maximum entropy reinforcement learning (Section \ref{sec:2-3-1-merl}) entropy is added directly into the objective function to be optimised, rather than using entropy as a constraint to disambiguate between multiple optimal policies. While the maximum entropy objective does yield a unique optimal policy (with respect to the maximum entropy objective), the policy is not guaranteed to align with the standard objective.
...
%     \todo{Rename this subsection to just Trial Based Search I think makes sense}
...
%             \bd{H-MCTS is used as an UCT style MCTS algorithm that is also designed using simple regret for comparison. (this was in intro to section 4.4, DENTS results section)} 
    

%         \todo{Should make sure to talk about all of the MCTS algorithms designed around simple regret. Confirm the claim that they all are really just some mix of uniform and UCT}

%         \todo{should talk about MCTS methods that use simple regret, specifically H-MCTS and probably BRUE etc}

%         \todo{Also talk about Trial-Based RTDP here}


%         % Something wrote at some point

%         \todo{introduce RENTS and TENTS outside of the introduction}
%         Entropy is widely used in the reinforcement learning literature, commonly introduced to promote exploration and discourage convergence to suboptimal deterministic policies \cite{ppo,deep_energy_policies,a3c,reinforce_ment}. MENTS (Section \ref{sec:2-4-4-ments}), RENTS and TENTS (Section \ref{sec:3-3-3-rents-some-tents}) are all MCTS algoritms that optimise for maximum entropy objectives. While RENTS and TENTS will be considered as baselines in empirical experiments, MENTS will be used to facilitate and provide discussion around the use of entropy and maximum entropy objective.

%         \todo{make sure H-MCTS, (BRUE), RENTS and TENTS baselines all introduced in related work section here.}
...
%     \todo{Mention the prior work that uses Point Estimates, and make a brief (informal) argument for why this is insufficient, and say that won't be considering this approach in this thesis, or even implementing it as baseline}


%     This paper make similar arguments to the ones I'm making in Ch5, should talk about it here too
%     % https://jmlr.org/papers/volume15/vanmoffaert14a/vanmoffaert14a.pdf



%     Claude suggestion on how to justify use of linear utilities in this thesis:
%     How I'd actually write this in the thesis. \todo{see comment}
% % I'd lead with (1), which is the most defensible and most specific to your technical contribution — it justifies the assumption in terms of what the assumption buys you. Then a paragraph of (2) to position the work within MORL more broadly. (3) is worth a sentence or two as a decision-theoretic grounding but can be compressed. (4) is a citation, not really a paragraph.
% % Something like:

% % *"This thesis restricts attention to linear utility functions of the form u(v⃗)=w⃗⋅v⃗u(\vec{v}) = \vec{w} \cdot \vec{v}
% % u(v)=w⋅v. This restriction is not merely a convenience: linear utilities are exactly the class for which the Bellman expectation operator commutes with scalarisation (a consequence of the characterisation of functions satisfying f(E[X⃗])=E[f(X⃗)]f(\mathbb{E}[\vec{X}]) = \mathbb{E}[f(\vec{X})]
% % f(E[X])=E[f(X)] as the affine functions). Consequently, they are the maximal class for which vector-valued dynamic programming reduces to scalar dynamic programming via a post-hoc argmax, and for which the convex coverage set suffices in place of the full Pareto coverage set. Broader utility classes either require tracking the entire PCS — generally non-convex and potentially exponentially larger — or abandoning the expectation-based formulation in favour of distributional return representations. The linear case additionally subsumes, as a family, the single-weight formulations standard in single-objective RL, and aligns with the dominant convention in the MORL literature [cite Roijers et al., 2013]."*

% % That's ~150 words and I think it covers the assumption pretty comprehensively for a thesis. You can expand with a concrete example (e.g. "a Chebyshev utility min⁡iwivi\min_i w_i v_i
% % mini​wi​vi​ can favour points strictly interior to the convex hull, so the CCS is no longer sufficient") if you want to make the point more tangible.


% % THIS IS IMPORTANT:
% % One thing worth flagging: you should also be honest about what the assumption rules out. The classic counterexample is risk-aversion over returns (where you care about variance, not just mean) — this is genuinely outside the scope, and it's worth a sentence acknowledging that rather than pretending linear utility is universal.
