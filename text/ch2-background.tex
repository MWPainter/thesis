...
\label{sec:2-1-mab}
    ...
    The Multi-Armed Bandit (MAB) problem \todo{mab book cite} is a foundational problem considered in decision theory. In the $K$-armed bandit problem, an agent must choose which of the $K$ arms to pull over multiple rounds. On each round, after pulling an arm, the agent receives a reward which is sampled from a probability distribution associated with the arm.
...
\label{sec:2-1-0-ucb}
    ...
    \textit{Regret} is a common metric used to assess the performance of algorithms for MAB problems, which is the difference between the cumulative reward obtained and the maximal expected reward that could be obtained with full knowledge of $\{f(i)\}$.
...
\label{sec:2-1-1-exploring-mab}
    ...
    \todo{update litrev table for all of the ch2 cites, and update writing about them if necessary (might be worth writing the theory results for exploring bandits)}
    ...
    Algorithms for EMAB problems can be assessed by considering the expected regret of following the recommendation policy for one round, known as \textit{simple regret}.
    \begin{defn}
...
\label{sec:2-1-2-contextual-mab}
    ...
    \todo{clean up this section, I'm too eepy right now to handle contextual brandits}
...
\label{sec:2-2-mdps}
    ...
    Throughout this thesis MDPs will be defined with \textit{terminal states}, which are states that, once reached. can never be left.
    \begin{defn}
        \label{def:2:terminal}
        A state $s\in\cl{S}$ is a \textnormal{terminal} state (or a \textnormal{trap} state), if the transition distribution always returns the same state (i.e. $p(s|s,a)=1$ for all actions $a$). The function $\terminal(s)$ returns a boolean in $\{\textnormal{True},\textnormal{False}\}$ which will be used to denote if a state $s$ is terminal or not.
    \end{defn}

    To define a strategy that an agent will follow in an MDP, an agent defines a \textit{policy}. A \textit{policy} maps each state to a probability distribution over the action space. A policy serves as the strategy that an agent or algorithm follows when acting in an MDP. Often it is desirable to define deterministic policies, which always produce the same action when given the same state, and can be represented as \textit{one-hot} distributions. 
