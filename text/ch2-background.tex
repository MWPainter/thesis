...
\label{sec:2-4-0-notation}
    ...
    \thtspp\ewe will operate by building a \textit{search tree} $\cl{T}$, built by running \textit{trials} from the root node until a stop signal is sent. Trees consist of \textit{decision nodes} that correspond to states in the search tree $s\in\cl{T}$, and \textit{chance nodes} that correspond to state-action pairs in the search tree $(s,a)\in\cl{T}$. Figure \ref{fig:2:tree_notation} shows how decision and chance nodes will be depicted as circles and diamonds in diagrams.
    \begin{defn}
    ...
    Because of the assumption that states and state-action pairs have a one-to-one correspondence with nodes in the search tree (Section \ref{sec:2-4-0-notation}), this thesis will often use $s$ and $(s,a)$ to refer to decision and chance nodes respectively. When neccessary to disambiguate between states, state-action pairs and nodes, the notation $\node(s)$ and $\node(s,a)$ will be used to indicate nodes explicitly. 
    ...
    Each decision and chance node will store some algorithm-dependent data, typically a \textit{value estimate} and a \textit{visit count}. $N(s)$ and $N(s,a)$ will denote the number of times $\node(s)$ and $\node(s,a)$ have been visited, or equivalently, the number of times $s$ and $(s,a)$ appear in trajectories sampled in \thtspp\ewe trials. $\dnodedata{s}$ is used to denote the set of values stored at node $\node(s)$, and $\cnodedata{s,a}$ for the set of values stored at node $\node(s,a)$. 
...
\label{sec:2-4-1-thts}
    ...
    \thtspp\ewe \cite{thtspp} is introduced in this subsection and provides a modular framework for defining and implementing MCTS algorithms. It extends the Trial-Based Heuristic Tree Search (THTS) schema \cite{thts} in a way that incorporates \mctsone\ewe algorithms and more modern MCTS algorithms that use a neural network or heuristic in place of a rollout simulation \cite{alphago,alphago_zero}. It is largely presented in a single-threaded manner, which is sufficient for the purposes of this thesis.

    Each trial will have an associated \textit{context}, which is a \textit{per-trial} key-value store that can be used to store any algorithm-dependent data. The algorithms defined in Sections \ref{sec:2-4-2-uct} and \ref{sec:2-4-3-ments} will not make use of this context, but it will be used in Chapters \ref{ch:5-chmcts} and \ref{ch:6-simplexmaps} for multi-objective MCTS algorithms where \textit{contextual tree search} is discussed. Beyond the scope of this thesis, contexts are necessary to be able to implement any MCTS algorithm that requires per-trial state, for example Partially Observable Monte Carlo Planning \cite{pomcp}, where a state is sampled from a \textit{belief state} at the beginning of each trial.
    \begin{defn}
    ...
    \begin{figure}
        ...
        \caption[Overview of one trial of \thtspp.]{Overview of one trial of \thtspp, where orange shows an example when \mctsmode\ewe is True, and blue shows an example when \mctsmode\ewe is False. From left to right: first a context is sampled, which stores any necessary per-trial state (not depicted) and the search tree at the beginning of the trial is shown; second shows the selection phase, where a trajectory is sampled and where dashed lines indicate any new nodes added; third shows that new nodes are initialised using the heuristic function $\Vinit$; and finally on the right, shows the backup phase, where the arrows directions are changed to show that information is being propogated back up the tree to update the values in the nodes (triangles). \todo{Add a ORANGE dashed arrow back from heuristic to selection to show the alternation when mcts mode is false.} \todo{update Vinit to Vheuristic in line with text}}
        \label{fig:2:thts}
    \end{figure}

    The main four phases of a \thtspp\ewe trial are (depicted in Figure \ref{fig:2:thts}):
    \begin{enumerate}
        \item  
            \textit{Context sampling} - a \textit{context} is sampled for the trial;
        \item 
            \textit{Selection phase} - a trajectory is sampled using a \textit{search policy}, any newly visited states (and state-action pairs) are added to $\cl{T}$ as decision nodes (and chance nodes);\footnote{Note that the \thtspp\ewe selection phase encapsulates both the selection and expansion phases of \mctsone\ewe.}
        \item 
            \textit{Heuristic phase} - any new nodes added in the selection phase are initialised using a \textit{heuristic function}; 
        \item 
            \textit{Backup phase} - value estimates in the search tree are updated, along the path sampled in the selection phase. 
    \end{enumerate}

    While the selection phase of THTS will sample a trajectory, alternating between selection and heuristic phases until a terminal state is reached, in \mctsone\ewe the selection phase ends once a a new decision node is added to the search tree. \thtspp\ewe uses a boolean flag \mctsmode\ewe to indicate which of these two criteria will be used to end the selection phase. Figure \ref{fig:2:thts} shows an example of a trial when \mctsmode\ewe is True (blue), and when it is False (orange).
    \begin{defn}
        \label{def:2:mctsmode}
        When \mctsmode\ewe is True, the selection phase will end once a new decision node is added to the search tree. When \mctsmode\ewe is False, the selection phase will continue until a terminal state is reached, or the planning horizon is reached.
    \end{defn} 

    There are two main benefits to running an algorithm in \mctsmode. The first is that it can use significantly less memory, which is a concern if using a large planning horizon and the tree search is going to run for a long time. The second is that if \mctsmode\ewe is used with an informative heuristic function, then it allows the algorithm to avoid wasting resources (time and memory) on parts of the search tree that are not promising. 
    
    In contrast, a random simulation is often used in \mctsone\ewe algorithms when no informative heuristic is available. In such cases, where memory allows, running with \mctsmode\ewe set to False can be beneficial, because states that would have been visited in the simulation phase of \mctsone\ewe are added to the tree and avoids throwing away potentially useful information. \todo{Add rollout add cite (see mcgs paper)}
    ...
    % }
    \todo{write about graph mode and add cite}
    - Graph mode
    ...
    \begin{description}
        ...
        \item[Action Selection (Search Policy):]
            \todo{add sample action function. Say that it will be specified as a}
            A policy $\pisearch$ used to \textit{select} actions at decision nodes;
        \item[(Optional) Outcome Sampler:]
            A function \sampleoutcome\ewe that samples outcomes from chance nodes. By default, this will sample a state from the transition distribution of the MDP: $s'\sim p(\cdot|s,a)$ and this default will always be used in this thesis;
        \item[Backup functions:]
            Two functions $\backupv$ and $\backupq$ which specifies how to update $\dnodedata{s}$ and $\cnodedata{s,a}$ respectively.
    \end{description}
    
    Parameters are set on a per-environment basis:
    \begin{description}
        \item[(Optional) Heuristic functions:]
            A functions $\Vinit$, $\Qinit$ and $\piinit$ are the heuristic functions. $\Vinit$ is used to initialise value estimates in $\dnodedata{s}$. $\Qinit$ and $\piinit$ can be used by the search policy to encorporate prior knowledge about unexplored actions;
    ...
    \todo{Add the weighting of heuristic funcitons in the backups}
    
    Given all of the modules and parameters are specified, the full operation of a trial in \thtspp\ewe is given below. Additionally, psuedocode is given in Listing \ref{lst:2:thts_trial}. \todo{This is not consistent with the text above right now, also specifiy initial tree is } $\cl{T}\leftarrow\{s_0\}$ \todo{somewhere}.
    \begin{enumerate}
        ...
        \item A trajectory is sampled $\tau_{:h}\sim\pisearch$ according to the search policy $\pisearch$ and \sampleoutcome. The search tree is updated to include any new nodes from the sampled trajectory, $\cl{T}\leftarrow\cl{T}\cup\tau_{:h}$.
        \begin{itemize}
            \item If running in \mctsmode, then $\tau_{:h}$ is such that $s_t\in\cl{T}$ for $t=0,\ldots,h-1$ and $s_h\not\in\cl{T}$, or $h=H_{\thtspp}$, or $s_h$ is terminal.
            \item If not running in \mctsmode, then $h=H_{\thtspp}$, or $s_h$ is terminal.
        \end{itemize}
        \item The heuristic value function $\Vinit$ is used to initialise the value of any new decision nodes added to the search tree.
        \item Finally, for the backup phase, the $\backupq$ and $\backupv$ functions are used to update the values of $\cnodedata{s_t,a_t}$ and $\dnodedata{s_t}$ for $t=h-1,\ldots,0$.
    \end{enumerate}
...
\label{sec:2-4-2-uct}
    ...
    \todo{Added this quickly when writing ch4}
