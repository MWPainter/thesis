...
\label{sec:4-5-theory}
    ...
    \subsection{MCTS As A Stochastic Process}
        ...
        \begin{align}
            \piments^k(a|s) &= (1-\lambdakments{k})\rhoments^k(a|s) + \frac{\lambdakments{k}}{|\cl{A}|}, 
                        \label{appeq:ments_soft_policy} \\ 
            \rhoments^k(a|s) &= \exp\left(\frac{1}{\alphaments}\left(\Qments\vis{N(s,a,k-1)}(s,a)-\Vments\vis{N(s,k-1)}(s)\right)\right). \\
        ...
        \todo{recommendation policies}

        The $k$th trajectory is sampled using the search policy: $\tau^k \sim \piments^k$. Letting $\tau^k=(s_0^k,a_0^k,r_0^k,...,s_{h-1}^k,a_{h-1}^k,r_{h-1}^k,s_{h}^k)$, the MENTS value estimates are computed using backups for $t=h-1, ..., 0$: \todo{either add the superscript k into every s and a, or update words to say implicit}
        \begin{align}
            \Vments\vis{N(s_t,k)}(s_t) &= 
                \alphaments \log \sum_{a\in\cl{A}} \exp \left(
                    \frac{1}{\alphaments}\Qments\vis{N(s_t,a,k)}(s_t,a) \right), \label{appeq:soft_v_backup} \\
            \Qments\vis{N(s_t,a_t,k)}(s_t,a_t) &= 
                R(s_t,a_t) + \sum_{s'\in\suc{s}{a}} \left( 
                    \frac{N(s',k)}{N(s_t,a_t,k)} \Vments\vis{N(s',k)}(s') \right). \label{appeq:soft_q_backup}
        \end{align}

        \todo{also want something to say that }
        %
        ...
        \todo{NEXT NEXT NEXT: clean things up for what written so far. Continue by first cleaning the above up. Then shitting out the below stuff. Then copying some of the lemmas and things. Add big gaps and block comments for the different things that we do.}



        Would like to do the generic every action visited infinitely often part first.
    ...
    \subsection{Lemmas (to organise an title better)}
        ...
        \begin{lemma} \label{lem:inf_often_action_select}
            Let $\rho^k$ be an arbitrary policy, and the following policy:
            \begin{align} 
        ...
        \end{lemma}
        \begin{proof}
            First consider that if $M(a,m)\not\rightarrow\infty$ then it is logically equivalent to say that there exists some $\ell$ such that from $a^\ell$ onwards that there is some action which is never sampled again. To prove the result, argue by contradiction, and suppose that there is some $\ell$ and $b\in\cl{A}$ such that:
            \begin{align}
            ...
            \end{align}
            %
            which is in contradiction to inequality (\ref{appeq:ioas_contradict}) that was assumed. Inequality (\ref{appeq:ioas_one}) holds because the factors in the product are increasing with respect to $k$. To see why the final limit equality (\ref{appeq:ioas_two}) holds, consider the simpler function $f(x)=\left(1-1/\log(x)\right)^x$. Taking logarithms and applying L'Hopital's rule generously gives:
            \begin{align}
        ...
        \begin{corollary} \label{cor:inf_often_action_select}
            For any MCTS algorithm using a search policy of the form $\pi^k(a)=(1-\lambda(k))\rho^k(a) + \lambda(k) \frac{1}{|\cl{A}|}$, all reachable states from the root node are visited infinitely often. Specifically, for any reachable $s\in\cl{S}$ is must be that $N(s,m)\raas\infty$ and $N(s,a,m)\raas\infty$ as $m\rightarrow\infty$.
        \end{corollary}
        \begin{proofoutline}
