...
\label{sec:4-5-theory}
    ...
    \subsection{Preliminaries - Exp}
        ...
        \subsection{General Q-value convergence result} \label{app:q_result}
            ...
            \end{corollary}
            \begin{proofoutline}
                Recall Equation (\ref{appeq:ns_sum}) that allows $N(s_{t+1},n)$ to be written as a sum of $N(s_t,a_t,n)$ indicator random variables: $N(s_{t+1},n) = \sum_{i\in T(s_t,a_t,n)} \one[s^i_{t+1} = s_{t+1}]$. Consider mapping the empirical transition distribution, with $N(s_t,a_t,n)=|T(s_t,a_t,n)|$ samples, to an appropriate catagorical distribution. Applying Theorem \ref{thrm:dkw_inequality} with $\varepsilon/2$ gives the result. \todo{also make saying replacing varepsilon in above result with half varepsilon word better.} \todo{clean up ``we'' in this proof} In the constructed cumulative distribution function we need to account for an error of $\varepsilon/2$ before any $s_{t+1}$, and another error of $\varepsilon/2$ after the same $s_{t+1}$, to account for an error of $\varepsilon$ in the probability mass function (see Figure \ref{fig:dkw_diag}).
            \end{proofoutline}
            ...
            \todo{HERE HERE HERE, FIRST THING TO DO IS PROOF READ THIS SECTION, SECOND THING TO DO IS MAKE THE FIG.}
