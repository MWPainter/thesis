...
This appendix provides the full definition of the DENTS stochastic process in Section \ref{appsec:dents_stoch_process} and the proofs for exponential convergence results for BTS and DENTS are given in Section \ref{sec:4-5-theory}



\section{The DENTS Stochastic Process}
\label{appsec:dents_stoch_process}

    The DENTS stochastic process formalises the DENTS algorithm as a stochastic process, which is defined as follows. This section uses the notation and assumptions established in \ref{sec:4-5-theory}. 
    
    All value estimates are initialised to zero for all $s\in\cl{S}$ and $a\in\cl{A}$:
    \begin{align}
        \mVdents{0}(s) = 0, \quad \mQdents{0}(s,a) = 0, \quad \mHVdents{0}(s) = 0, \quad \mHQdents{0}(s,a) = 0.
    \end{align}

    On the $n$th trial, a trajectory is sampled using the search policy $\tau^n \sim \mpidents{n}$. Where for all $s,a\in\cl{S}\times\cl{A}$, the search policy is defined as:
    %
    \begin{align}
        \mQdentssft{N^n(s,a)}(s,a) &= \mQdents{N^n(s,a)}(s,a) + \betadents(N^n(s))\mHQdents{N^n(s,a)}(s,a), \label{appeq:dents_q_sft} \\
        \mrhodents{m}(a|s) &= \frac{
                \exp\left(\frac{\mQdentssft{N^n(s,a)}(s,a)}{\alphadents(N^m(s))}\right)
            }{
                \sum\limits_{a'\in\cl{A}} \exp\left(\frac{\mQdentssft{N^n(s,a')}(s,a')}{\alphadents(N^n(s))}\right)
            }.  \label{appeq:full_rho} \\
        \lambda^m(s,\epsdents) &= \min\left(1, \frac{\epsdents}{\log(e+N^{n-1}(s))}\right),  \\
        \mpidents{n}(a|s) &= (1-\mlambdadents{n})\mrhodents{n}(a|s) + \frac{\mlambdadents{n}}{|\cl{A}|}. 
            \label{appeq:dents_search_policy}  \\
    \end{align}

    Letting $\tau^n=(s^n_0,a^n_0,r^n_0,\ldots,s^n_{H-1},a^n_{H-1},r^n_{H-1},s^n_{H})$, the DENTS value and entropy estimates are computed as follows for $t=H-1,\ldots,0$:
    %
    \begin{align}
        \mQdents{N^m(s_t,a_t)}(s_t,a_t) &= 
            R(s_t,a_t) + \sum_{s' \in \suc{s_t}{a_t}} \left( 
                \frac{N^m(s')}{N^m(s_t,a_t)} \mVdents{N^m(s')}(s') \right), 
            \label{appeq:dp_q_backup} \\ 
        \mVdents{N^m(s_t)}(s_t) &=\max_{a\in\cl{A}} \mQdents{N^m(s_t,a)}(s_t,a), 
            \label{appeq:dp_v_backup} \\
        \mHQdents{N^m(s_t,a_t)}(s_t,a_t) &= 
            \sum_{s'\in \suc{s_t}{a_t}} \frac{N^m(s')}{N^m(s_t,a_t)} \mHVdents{N^m(s')}(s'), \\
        \mHVdents{N^m(s_t)}(s_t) &= 
            \cl{H}(\mpidents{m}(\cdot | s_t)) 
                + \sum_{a\in\cl{A}} \mpidents{m}(a_t|s_t)\mHQdents{N^m(s_t,a_t)}(s_t,a_t).
    \end{align}

    Recall that the visit counts are \textit{defined} as a sum of indicator random variables using the sampled trajectories:
    \begin{align}
        N^n(s_t) &= \sum_{i=1}^n \one[s^i_t = s_t], \\
        N^n(s_t,a_t) &= \sum_{i=1}^n \one[s^i_t = s_t \cap a^i_t = a_t].
    \end{align}

    And note that if $s\not\in\tau^n$, then $N^n(s)=N^{n-1}(s)$, and similarly if $(s,a)\not\in\tau^n$, then $N^n(s,a)=N^{n-1}(s,a)$. More specifically, this implies, for example, that if $s\not\in\tau^n$ then:
    \begin{align}
        \mVdents{N^n(s)}(s) = \mVdents{N^{n-1}(s)}(s).
    \end{align}

    Finally, the maximum value and most visited recommendation policies after $n$ trials are respectively defined as:
    \begin{align}
        \mpsidents{n}(s) &= \argmax_{a\in\cl{A}}\mQdents{N^n(s,a)}(s,a), \\
        \mmvdents{n}(s) &= \argmax_{a\in\cl{A}} N^n(s,a).
    \end{align}
    ...
    %     \odot{Copy DENTS after its written. Swap out notation and backups.}
    ...
    \subsection{The (AR-)BTS process.}
        ...
        The \mbox{(AR-)BTS} process is a special case of the \mbox{(AR-)DENTS} process when $\betadents(x)=0$. 
    ...
    % \odot{Move to preliminaries. Something like: these intermediate values are useful in the proofs for exponential convergence results.}    


        % \odot{From the neurips, also wrote the following (everything left in this subsubsection is copied and cleaned up, but probably needs a proof read)} Let $\mVrhodents{N(s)}(s)$ be defined as the value:
        % %
        % \begin{align}
        %     \mVrhodents{N(s)}(s) = &
...
\label{appsec:exp}
    ...
    Note to examiners/anyone reading this: I'm sorry I didn't find time before submission to clean up the proofs below this point. It's definitely not worth your time looking at it sorry.




    % \odot{This section give proofs for exponential convergence results for DENTS and MENTS}, it is structured as follows:
    \begin{enumerate}
    ...
    % \odot{Make sure assumptions and conditions stated. Anything that depends on the min prob needs to have the lower/upper bounds on the decay functions for example.}
    ...
    \label{appsec:exp-prelim}
        ...
        % \odot{<beg> dump}
            ...
            % \odot{This section contains things related to exponential bounds, which might move to appendix}
            ...
            % \odot{change m to n, and use mth when need to reason about prev trials (whole subsection)}





            Firstly it will be that the union of exponentially unlikely events is also exponentially unlikely, and that the intersection of exponentially likely events is exponentially likely. Although this is a special case of the union bound % \odot{ref?} %
            % \odot{it is stated here so that it can be referenced, because this fact is used regularly }
            %
            % \odot{Some comment about how this is just a special case of the union bound (and ref that?) and also }
            %
            ...
            The following two lemma's show that the MENTS and DENTS processes always have a minimum probability of picking any action. %\odot{not in good headspace writing up these two lemmas. Do a clean up, double checking maths and that have eliminated using 'we' and so on}
            %
            ...
            %
            \begin{proofoutline}
                % \odot{Fix N having to be N(s,m) not N(s) etc}
                ...
                \end{align}
                % \odot{clean up with respect to init function}
    ...
    \label{appsec:q_result}
        ...
        Now that Corollary \ref{appcor:4:bound_transition_distribution} can be used to bound  the empirical transition distribution to the true transition distribution,  a general purpose concentration inequality for Q-values can be proved for use in later proofs.
        ...
        \begin{proof}
            ...
            And recall that for any $p(s_{t+1}|s_t,a_t) > \varepsilon_2>0$, using Corollary \ref{appcor:4:bound_transition_distribution} that:
            \begin{align}
