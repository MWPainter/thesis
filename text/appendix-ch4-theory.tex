...
This appendix provides the full definition of the DENTS stochastic process in Section \ref{appsec:dents_stoch_process} and the proofs for exponential convergence results for BTS and DENTS are given in Section \todo{ref}.
...
\label{appsec:dents_stoch_process}
    ...
    The DENTS stochastic process formalises the DENTS algorithm as a stochastic process, which is defined as follows. This section uses the notation and assumptions established in \todo{ref to ch4}. 

    \todo{make sure handle initv and initq properly in the appendix}
    ...
    \begin{align}
        ...
        \mrhodents{m}(a|s) &= \frac{
                \exp\left(\frac{\mQdentssft{N^n(s,a)}(s,a)}{\alphadents(N(s,m))}\right)
            }{
    ...
    \todo{Move to preliminaries. Something like: these intermediate values are useful in the proofs for exponential convergence results.}    
...
\label{appsec:exp}
    ...
    This section provides the proofs of exponential convergence results for the DENTS, AR-DENTS, BTS and AR-BTS stochastic processes. It is structured similarly to Section \ref{sec:4-5-theory} as follows:
    \begin{enumerate}
        \item \todo{todo}
    \end{enumerate}
    ...
    \label{appsec:q_result}
        ...
        \todo{<beg> dump (stuff put in main text)}
            ...
            \end{figure}
            %
            \begin{corollary} \label{cor:4:bound_transition_distribution}
        ...
        Now that Corollary \ref{cor:4:bound_transition_distribution} can be used to bound  the empirical transition distribution to the true transition distribution,  a general purpose concentration inequality for Q-values can be proved for use in later proofs.
        ...
        \begin{proof}
            ...
            And recall that for any $p(s_{t+1}|s_t,a_t) > \varepsilon_2>0$, using Corollary \ref{cor:4:bound_transition_distribution} that:
            \begin{align}
