...
\label{sec:1-1-overview}
    ...
    \todo{Motivate MO settings similarly to MORL survey, and reproduce some of the examples they use}

    \todo{Motivation with respect to: In the decision support scenario, an algorithm computes a \textit{solution set} consisting of multiple \textit{possibly optimal} solutions, from which a user then picks their most preferred option to be used. This scenario is useful when a user's preferences over the multiple objectives is unknown or difficult to specify in advance. (In Ch2.5 first para)}
...
\label{sec:1-2-contributions}
    ...
    \begin{itemize}
        ...
        \item Contextual Zooming and CHMCTS (designed for \contextq, results \moevalq)
        \item Simplex maps (\contextq, \moscalabilityq)
